...
// $regex find Age starts with 'o'
db.customers.find({ Age: { $regex: /^o/ } });
//end with g
db.customers.find({ Age: { $regex: /g$/ } });
//case sensitive (use i)
db.customers.find({ Age: { $regex: /G$/i } });
db.customers.find({ Age: { $regex: /^O/i } });
