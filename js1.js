...
//             if(!data.ok) throw new Error("got error")
...
// async function fetchWithLimit(urls, retries){
//     const result = []
//     for (const url of urls) {
//         const data = await fetchWithRetries(url, retries)
//         result.push(data)
//     }
//     return result
// }
...
* Group items by a property name or a mapping function.

* @param {any[]} items

* @param {string | ((x:any) => string | number)} key
...
function groupBy(items, key) {
  ...
  // TODO: implement
  const data  = items.reduce((acc,obj)=>{

    if(acc[obj.role=="admin"]){
        acc[obj.role] = obj
    }else if(acc[obj.role=="user"]){
        acc[obj.role] = obj
    }else{
        acc[obj.role == undefined] = obj
    }
...
// ---- Quick demo/tests the candidate can run ----
...
//   undefined: [{Eve...}] // acceptable or omit if you choose to skip missing
